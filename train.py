...
def train_agent(env, agent, n_episodes=100, max_steps=1000, eval_freq=10):
    """Main training loop"""
    # Track metrics
    episode_rewards = []
    portfolio_values = []
...
def plot_training_progress(rewards, latest_portfolio_values):
    ...
    plt.tight_layout()
def evaluate_performance(price_data, agent):
    env_eval = MarketEnv(price_data)
    ...
    model_returns     = np.array(env_eval.portfolio_returns_history)
    model_cum_growth  = np.cumprod(1 + model_returns)

    daily_rets         = price_data[1:] / price_data[:-1] - 1
    n                  = price_data.shape[1]
    baseline_rets      = daily_rets.dot(np.ones(n) / n)
    baseline_cum_growth = np.cumprod(1 + baseline_rets)

    plt.figure(figsize=(10, 6))
    plt.plot(model_cum_growth,     label="TD3 Agent")
    plt.plot(baseline_cum_growth,  label="Equal‑Weight")
    plt.title("Cumulative Growth: Model vs. Equal‑Weight")
    ...
    model_total    = model_cum_growth[-1]     - 1
    baseline_total = baseline_cum_growth[-1]  - 1
    alpha          = model_total - baseline_total

    print(f"Model Total Return:    {model_total*100:.2f}%")
    print(f"Equal‑Weight Return:   {baseline_total*100:.2f}%")
    print(f"Alpha (Excess Ret):    {alpha*100:.2f}%")
...
if __name__ == "__main__":
    ...
    try:
        ...
        evaluate_performance(stock_data, agent)
    except Exception as e:
